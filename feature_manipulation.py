...
def spec_augment(sample, max_mask_pct=0.1, n_time_masks=1):
    aug_sample = np.copy(sample)
...
def main():
    ...
    mel_spectrogram = librosa.feature.melspectrogram(y=y, sr=sr)
    mel_spectrogram_db = librosa.power_to_db(mel_spectrogram, ref=np.max)
    aug_spec = spec_augment(mel_spectrogram_db)
    aug_spec = time_shift(aug_spec, 20)

    fig, ax = plt.subplots()

    img = librosa.display.specshow(aug_spec, x_axis='time', y_axis='mel', sr=sr, fmax=8000, ax=ax)
